...
Data_path<-'D:\\Academic\\Projects\\2CRMDH\\Data\\New_Data'
library(gmm)
library(dplyr)
setwd(Code_path)
...
for (q in 1:length(name_list)){
  ...
  #读取所选中股票数据
  symbol=name_list[q]
  last_name=paste(symbol,".csv",sep="")
  family_name=paste(Data_path,last_name,sep="\\")
  data=read.csv(family_name)[,2:5]
  points(data$volume,type='l',col='red')
  plot(data$volume,type='l',col='red')
  points(data1$V4,type='l')
  #筛选数据
  days_num=nrow(data)
  start_index=241 #我们用241天之后的数据
    # which.max(data[,1]-20120900>0) #依论文所说，筛选9月份之后的数据
  ...
  #为MMDH模型估计获取初始值
  data_tmp=select(data[start_index:(start_index+Lw-1),2:4],-'RV')
  fit_tmp=gmm(g=g1,x=data_tmp,t0=c(rbar=0.1,omega=0.1,beta=0.7,sigmau=0.4,c=0.2,m0=1,m1=1),
  ...
  for (day_id in 1:fore_num){
    ...
    for (k in 1:Lw){
      ...
      #这是在选取什么数据
      data_HAR[k,1]=data[(start_index+k+day_id-1-1),3] # 当天的价格
      data_HAR[k,2]=data[(start_index+k+day_id-1-2),3] # 前一天的价格
      data_HAR[k,3]=mean(data[(start_index+k+day_id-1-6):(start_index+k+day_id-1-3),3]) #前面五天到前面两天的均值价格
      data_HAR[k,4]=mean(data[(start_index+k+day_id-1-23):(start_index+k+day_id-1-7),3]) # 前面22天到前面6天的均值价格
